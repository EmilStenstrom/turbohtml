...
class TextHandler(TagHandler):
    ...
    def handle_text(self, text, context):
        ...
        if context.post_adoption_reconstruct_pending:
            ...
            if context.document_state == DocumentState.IN_BODY and not in_template_content(context):
    ...
    def _handle_pre_text(
        ...
        if decoded_text:
            if parent and needs_foster_parenting(parent) and decoded_text.strip() and not in_template_content(context):
    ...
    def handle_start(
        ...
        if token.tag_name == "p" and context.current_parent:
            ...
            if table_ancestor and not context.open_elements.contains(table_ancestor):
                body_node = (
                    get_body(self.parser.root) or ensure_body(self.parser.root, context.document_state, self.parser.fragment_context)
                )
                if body_node:
                    context.move_to_element(body_node)
                    self.debug(
