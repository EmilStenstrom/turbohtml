import re
from typing import TYPE_CHECKING, Optional, Tuple, List, Protocol, Callable

from turbohtml.node import Node
from turbohtml.context import ParserState, ParseContext
from turbohtml.tokenizer import HTMLToken
from turbohtml.constants import AUTO_CLOSING_TAGS, BLOCK_ELEMENTS, CLOSE_ON_PARENT_CLOSE, HEAD_ELEMENTS, HEADING_ELEMENTS, HTML_ELEMENTS, RAWTEXT_ELEMENTS, SVG_CASE_SENSITIVE_ELEMENTS, ADOPTION_FORMATTING_ELEMENTS, TABLE_ELEMENTS, FORMATTING_ELEMENTS, VOID_ELEMENTS
...
class TextHandler(TagHandler):
    ...
    def handle_text(self, text: str, context: 'ParseContext') -> bool:
        ...
        if last_child and last_child.tag_name == '#text':
            self.debug(f"TextHandler: merging with previous text node '{last_child.text_content}'")
...
class TableTagHandler(TagHandler):
    ...
    def handle_start(self, token: 'HTMLToken', context: 'ParseContext', end_tag_idx: int) -> bool:
        ...
        if context.state == ParserState.IN_TABLE:
            ...
            in_cell = context.current_parent.tag_name in ['td', 'th'] or context.current_parent.find_ancestor(lambda n: n.tag_name in ['td', 'th'])
    ...
    def handle_end(self, token: 'HTMLToken', context: 'ParseContext') -> bool:
        ...
        if tag_name == 'table':
            ...
            if not context.current_parent.find_ancestor(tag_name, stop_at_boundary=True):
            ...
            if temp_parent:
                ...
                if tag_name in TABLE_ELEMENTS:
                    ...
                    target_parent = target_parent.find_ancestor(lambda n: n.tag_name not in FORMATTING_ELEMENTS)
                
                # If we're still in a table cell or template after closing the tag, stay there
                current_container = target_parent.find_ancestor(lambda n: n.tag_name in ['td', 'th', 'template'])
    ...
    def handle_end(self, token: 'HTMLToken', context: 'ParseContext') -> bool:
        ...
        if token.tag_name in ('ul', 'ol'):
            ...
            if not list_container:
                ...
                while current and current != self.parser.root:
                    ...
                    if current.tag_name == token.tag_name:
                        ...
                        self.debug(f"Found list container in ancestors: {list_container}")
                        break
                    current = current.parent
            ...
            if list_container:
                ...
                self.debug(f"Moving to list container's parent: {list_container.parent}")
...
class HeadingTagHandler(TagHandler):
    ...
    def handle_start(self, token: 'HTMLToken', context: 'ParseContext', end_tag_idx: int) -> bool:
        ...
        current = context.current_parent.find_ancestor(lambda node: node.tag_name in HEADING_ELEMENTS)
        if current:
...
class AutoClosingTagHandler(TagHandler):
    ...
    def handle_start(self, token: 'HTMLToken', context: 'ParseContext', end_tag_idx: int) -> bool:
        ...
        if (token.tag_name in BLOCK_ELEMENTS and 
            ...
            self.debug(f"Created block {new_block} inside formatting element {current_fmt}")
...
class AdoptionAgencyHelper:
    ...
    def handle_formatting_boundary(context: 'ParseContext', parser: 'ParserInterface') -> Tuple[Node, List[Node]]:
        ...
        while temp and temp.tag_name != 'body':
            if temp.tag_name in ADOPTION_FORMATTING_ELEMENTS:
                formatting_elements.insert(0, temp)
                parser.debug(f"Found formatting element: {temp.tag_name} with children: {[c.tag_name for c in temp.children]}")
            temp = temp.parent
...
class ForeignTagHandler(TagHandler):
    ...
    def should_handle_text(self, text: str, context: 'ParseContext') -> bool:
        ...
        return (context.current_context in ('svg', 'math') and 
